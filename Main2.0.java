...
public class Main {
    ...
    private static void load_orders_2(Timestamp log_time, Date import_time, Date export_time, Date retrieval_start_time, Date delivery_finish_time,
                                    String item_name, String container_code, String ship_name, String retrieval_courier, String delivery_courier,
                                    String retrieval_city, String delivery_city, String import_city, String export_city) throws SQLException {
        if (to_orders != null) {
            to_orders.setTimestamp(1, log_time);
        ...
        try (BufferedReader infile = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((line = infile.readLine()) != null) {
                ...
                if (cnt >= 2)
                {
                    ...
                    if(!set_ship.contains(ship_name))
                    {
                        if (to_ship != null) {
                            to_ship.setString(1, ship_name);
                            to_ship.addBatch();
                            set_ship.add(ship_name);
                        }
                    }
                    if(!parts[11].equals("")&&!set_courier.contains(delivery_courier))
                    {
                        if (to_courier != null) {
                            to_courier.setString(1, delivery_courier);
                            to_courier.setString(2, delivery_courier_gender);
                            to_courier.setString(3, delivery_courier_phone_number);
                            if(delivery_courier_age!=null)
                            {
                                to_courier.setInt(4, delivery_courier_age);
                            }
                            to_courier.addBatch();
                            set_courier.add(delivery_courier);
                        }
                    }
                    if(!set_courier.contains(retrieval_courier))
                    {
                        if (to_courier != null) {
                            to_courier.setString(1, retrieval_courier);
                            to_courier.setString(2, retrieval_courier_gender);
                            to_courier.setString(3, retrieval_courier_phone_number);
                            if(retrieval_courier_age!=null)
                            {
                                to_courier.setInt(4, retrieval_courier_age);
                            }
                            to_courier.addBatch();
                            set_courier.add(retrieval_courier);
                        }
                    }
                    if(!set_city.contains(delivery_city))
                    {
                        if (to_city != null) {
                            to_city.setString(1, delivery_city);
                            to_city.addBatch();
                            set_city.add(delivery_city);
                        }
                    }
                    if(!set_city.contains(retrieval_city))
                    {
                        if (to_city != null) {
                            to_city.setString(1, retrieval_city);
                            to_city.addBatch();
                            set_city.add(retrieval_city);
                        }
                    }
                    if(!set_city.contains(item_export_city))
                    {
                        if (to_city != null) {
                            to_city.setString(1, item_export_city);
                            to_city.addBatch();
                            set_city.add(item_export_city);
                        }
                    }
                    if(!set_city.contains(item_import_city))
                    {
                        if (to_city != null) {
                            to_city.setString(1, item_import_city);
                            to_city.addBatch();
                            set_city.add(item_import_city);
                        }
                    }
                    if(!set_container.contains(container_code))
                    {
                        if (to_container != null) {
                            to_container.setString(1, container_code);
                            to_container.setString(2, container_type);
                            to_container.addBatch();
                            set_container.add(container_code);
                        }
                    }
                    if(!set_item.contains(item_name))
                    {
                        if (to_item != null) {
                            to_item.setString(1, item_name);
                            to_item.setString(2, item_type);
                            ...
                            to_item.setDouble(4, item_import_tax);
                            to_item.setDouble(5, item_export_tax);
                            to_item.addBatch();
                            set_item.add(item_name);
                        }
                    }
                    if(!set_company_ship.contains(company_name+ship_name))
                    {
                        if (to_company_ship_detail != null) {
                            to_company_ship_detail.setString(1, company_name);
                            to_company_ship_detail.setString(2, ship_name);
                            to_company_ship_detail.addBatch();
                            set_company_ship.add(company_name+ship_name);
                        }
                    }
                    if(!parts[11].equals("")&&!set_company_courier.contains(company_name+delivery_courier))
                    {
                        if (to_company_courier_detail != null) {
                            to_company_courier_detail.setString(1, company_name);
                            to_company_courier_detail.setString(2, delivery_courier);
                            to_company_courier_detail.addBatch();
                            set_company_courier.add(company_name+delivery_courier);
                        }
                    }
                    if(!set_company_courier.contains(company_name+retrieval_courier))
                    {
                        if (to_company_courier_detail != null) {
                            to_company_courier_detail.setString(1, company_name);
                            to_company_courier_detail.setString(2, retrieval_courier);
                            to_company_courier_detail.addBatch();
                            set_company_courier.add(company_name+retrieval_courier);
                        }
                    }
                    if(!set_berth.contains(ship_name+delivery_city))
                    {
                        if (to_berth != null) {
                            to_berth.setString(1, ship_name);
                            to_berth.setString(2, delivery_city);
                            to_berth.addBatch();
                            set_berth.add(ship_name+delivery_city);
                        }
                    }
                    if(!set_berth.contains(ship_name+retrieval_city))
                    {
                        if (to_berth != null) {
                            to_berth.setString(1, ship_name);
                            to_berth.setString(2, retrieval_city);
                            to_berth.addBatch();
                            set_berth.add(ship_name+retrieval_city);
                        }
                    }
                    if(!set_berth.contains(ship_name+item_export_city))
                    {
                        if (to_berth != null) {
                            to_berth.setString(1, ship_name);
                            to_berth.setString(2, item_export_city);
                            to_berth.addBatch();
                            set_berth.add(ship_name+item_export_city);
                        }
                    }
                    if(!set_berth.contains(ship_name+item_import_city))
                    {
                        if (to_berth != null) {
                            to_berth.setString(1, ship_name);
                            to_berth.setString(2, item_import_city);
                            to_berth.addBatch();
                            set_berth.add(ship_name+item_import_city);
                        }
                    }
                    try {
                        load_orders_2(log_time,item_import_time,item_export_time, retrieval_start_time,delivery_finish_time,
                                item_name,container_code,ship_name,retrieval_courier,delivery_courier,
                                retrieval_city,delivery_city,item_import_city,item_export_city);
                    } catch (SQLException sqlException) {}
                    if(cnt%Batch_size==0)
                    {
                        to_company.executeBatch();
                        to_ship.executeBatch();
                        to_city.executeBatch();
                        to_courier.executeBatch();
                        to_container.executeBatch();
                        to_item.executeBatch();
                        to_company_ship_detail.executeBatch();
                        to_company_courier_detail.executeBatch();
                        to_berth.executeBatch();
                        to_orders.executeBatch();

                        to_company.clearBatch();
                        to_ship.clearBatch();
                        to_city.clearBatch();
                        to_courier.clearBatch();
                        to_container.clearBatch();
                        to_item.clearBatch();
                        to_company_ship_detail.clearBatch();
                        to_company_courier_detail.clearBatch();
                        to_berth.clearBatch();
                        to_orders.clearBatch();
                    }
                }
            }
            if(cnt%Batch_size==0)
